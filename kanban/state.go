...
func (s *State) CleanupStaleRunningAgents(maxRunDuration time.Duration) int {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *State) CleanupOrphanedRunningAgents() int {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *State) IsAgentRunning(ticketID, agentType string) bool {
	s.mu.RLock()
	defer s.mu.RUnlock()
