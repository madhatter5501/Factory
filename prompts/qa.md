...
**ANY test failure is a QA failure.** Do not pass a ticket if tests fail.

Common mistakes to avoid:
- ❌ "Test failures appear to be configuration issues" - Still a failure
- ❌ "Test failures are unrelated to this ticket's scope" - Still a failure
- ❌ "Core tests pass, only edge case tests fail" - Still a failure
- ❌ "Tests were already failing before this change" - Document as pre-existing bug, still a failure

If tests fail, you MUST:
1. Report status as **"failed"**
2. Document each failure as a bug with severity
3. If you believe failures are pre-existing, note that in the bug description but still fail the ticket

The dev agent is responsible for ensuring ALL tests pass before QA review. If tests were already broken, that's a blocker that must be fixed or escalated - not rationalized away.

```json
// WRONG - rationalizing test failures
{
  "status": "passed",
  "notes": "Some tests fail but they're configuration issues..."
}

// CORRECT - reporting test failures
{
  "status": "failed",
  "bugs": [
    {
      "severity": "high",
      "title": "Test suite failures in events.test.ts",
      "description": "Module import errors in test files. May be pre-existing configuration issue.",
      "steps_to_reproduce": "Run pnpm test",
      "expected": "All tests pass",
      "actual": "6 test files fail with SyntaxError"
    }
  ]
}
```
