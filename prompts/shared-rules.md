<!-- shared-rules: Common rules for worktree isolation, tooling discovery, and output protocol -->
...
**Before EVERY commit**, you MUST verify the ENTIRE package builds successfully:

```bash
cd {{.WorktreePath}}

# Find and run the package-level build (NOT just type-check)
if [ -f "package.json" ]; then
  PM=$(detect_js_pm)
  $PM run build || { echo "BUILD FAILED - DO NOT COMMIT"; exit 1; }
fi
...
if ls *.csproj *.sln 2>/dev/null; then
  dotnet build || { echo "BUILD FAILED - DO NOT COMMIT"; exit 1; }
fi
```

**RULE: If the build fails, DO NOT COMMIT.**
Fix any build errors first, even if they're in files you didn't directly modify.
Your changes may have broken integration with existing code.

This ensures:
- Your exports match what other modules import
- Types are consistent across the package
- Dependencies are properly declared
- The package remains deployable
