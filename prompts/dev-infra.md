...
### 6. Verify All Tests/Validation Pass

**CRITICAL: You MUST ensure all tests and validations pass before committing.**
...
if [ -f "main.tf" ] || compgen -G "*.tf" > /dev/null; then
  terraform validate
  terraform plan  # Review the plan for issues
fi

# Kubernetes
if compgen -G "*.yaml" > /dev/null || compgen -G "*.yml" > /dev/null; then
  # Validate manifests
  kubectl apply --dry-run=client -f . 2>/dev/null || true
fi
...
if [ -f "Chart.yaml" ]; then
  helm lint .
  helm template . | kubectl apply --dry-run=client -f - 2>/dev/null || true
fi

# If there are any test files, run them
if [ -f "go.mod" ]; then
  go test ./...
fi
```
...
2. **Do NOT commit with failing validations** - QA will reject this
3. **If issues are pre-existing**, you must fix them or document in ticket (but still fix them)

### 7. Commit

Only commit if ALL checks pass (validation + tests):
